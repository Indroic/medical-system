...
colors:
  ...
  clinical-plum: "oklch(0.50 0.19 338)"
  clinical-plum-foreground: "oklch(0.99 0 0)"
  ...
  indigo-signal: "oklch(0.58 0.16 264)"
  indigo-signal-foreground: "oklch(0.99 0 0)"
  porcelain-blush-bg: "oklch(0.985 0.004 340)"
  pure-white-surface: "oklch(1 0 0)"
  ink-plum-foreground: "oklch(0.22 0.02 340)"
  dust-mauve-muted: "oklch(0.52 0.015 340)"
  hairline-blush-border: "oklch(0.90 0.01 340)"
  umbra-bg-dark: "oklch(0.16 0.006 340)"
  carbon-panel-surface-dark: "oklch(0.21 0.008 340)"
  chalk-foreground-dark: "oklch(0.96 0.006 340)"
  smoke-mauve-muted-dark: "oklch(0.68 0.02 340)"
  hairline-dusk-border-dark: "oklch(0.30 0.012 340)"
typography:
  display:
...
components:
  button-primary:
    backgroundColor: "{colors.clinical-plum}"
    textColor: "{colors.clinical-plum-foreground}"
  ...
  button-secondary:
    ...
    textColor: "{colors.ink-plum-foreground}"
    rounded: "{rounded.buttons}"
    padding: "10px 20px"
  badge-success:
  ...
  badge-info:
    ...
    textColor: "{colors.indigo-signal-foreground}"
    rounded: "{rounded.badges}"
    padding: "2px 10px"
  card:
    backgroundColor: "{colors.pure-white-surface}"
...
The palette is genuinely rich (clinical plum for brand and primary action, indigo for informational state, vital green / amber / critical red for risk and status), but it stays calm because each color is scoped to a specific, learnable meaning, the way Stripe's dashboard uses a rich palette without ever feeling loud. Surfaces stay quiet — pure white cards floating on a whisper-tinted page — so the color budget is spent entirely on things the clinician needs to act on.

**Key Characteristics:**
- A single confident brand color (Clinical Plum) carries identity in nav, primary actions, and links — never sprayed across backgrounds.
...
### Primary
- **Clinical Plum** (`oklch(0.50 0.19 338)`): The brand color. Used for primary buttons, active nav state, links, focus rings, and the loading/progress indicators. Never used as a page or card background — it appears as filled controls and text, not as atmosphere.

### Secondary
- **Indigo Signal** (`oklch(0.58 0.16 264)`): The informational accent — distinct in both hue and lightness from Clinical Plum. Used for "in progress / informational" states (e.g. a study `EN_ANALISIS`), secondary chips, and any UI that needs a second brand-adjacent color without being mistaken for the primary action.
...
### Neutral
- **Porcelain Blush** (`oklch(0.985 0.004 340)`) — page background (light). A near-white with a whisper of the brand hue, never a competing warm/cream cast.
...
- **Ink Plum** (`oklch(0.22 0.02 340)`) — primary text (light).
- **Dust Mauve** (`oklch(0.52 0.015 340)`) — secondary/muted text (light).
- **Hairline Blush** (`oklch(0.90 0.01 340)`) — borders and dividers (light).
- **Umbra** (`oklch(0.16 0.006 340)`) — page background (dark).
- **Carbon Panel** (`oklch(0.21 0.008 340)`) — surface/card background (dark).
- **Chalk** (`oklch(0.96 0.006 340)`) — primary text (dark).
- **Smoke Mauve** (`oklch(0.68 0.02 340)`) — secondary/muted text (dark).
- **Hairline Dusk** (`oklch(0.30 0.012 340)`) — borders and dividers (dark).

### Named Rules
**The Meaning-Only Rule.** Vital Green, Amber Alert, Critical Red, and Indigo Signal are reserved exclusively for risk level and study status. If a new UI element needs a color and it isn't communicating risk or status, reach for Clinical Plum, or don't add color at all.
...
- **Primary:** Clinical Plum fill, white text, `10px 20px` padding. Used for the one primary action per screen ("Nuevo estudio", "Acceder", "Crear administrador").
...
- **Hover / Focus:** Background shifts via HeroUI's built-in `-hover` color-mix tokens; focus ring uses `--focus` (= Clinical Plum) at 2px offset.
...
### Inputs / Fields
- **Style:** Field background matches surface; 1px `field-border` (Hairline Blush/Dusk) at 10px radius — visible outline by default, since ambiguous form boundaries are a real risk in a clinical data-entry context.
- **Focus:** Border shifts to Clinical Plum via `--field-border-focus`.
- **Error:** Border and helper text shift to Critical Red.

### Navigation
- **Style:** Left sidebar (240px), Porcelain Blush/Umbra background, hairline right border. Active item: Clinical Plum soft-fill background + plum text + plum icon. Inactive: muted text, hover to a neutral surface-hover tint (never colored — color is reserved for the active state so it stays legible as "you are here").

## 6. Do's and Don'ts

### Do:
- **Do** reserve Vital Green / Amber Alert / Critical Red / Indigo Signal strictly for risk level and study status — never for decoration, marketing accents, or arbitrary UI variety.
...
- **Do** use Clinical Plum for the single primary action on a screen; if two buttons compete for plum, one of them should be a secondary/ghost button instead.
