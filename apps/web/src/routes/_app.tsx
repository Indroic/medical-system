...
function AppLayout() {
  ...
  return (
    <div className="flex flex-col lg:flex-row h-svh bg-obsidian font-sans overflow-hidden">
      {/* Barra superior móvil */}
      <header className="flex lg:hidden h-14 items-center justify-between border-b border-charcoal bg-obsidian px-4 shrink-0">
        ...
        <button
          ...
          className="p-1.5 text-smoke hover:text-snow hover:bg-ash rounded-lg transition-colors"
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-50 flex lg:hidden">
          {/* Backdrop */}
          <div 
            className="fixed inset-0 bg-obsidian/85 backdrop-blur-sm transition-opacity duration-300"
            onClick={() => setIsMobileMenuOpen(false)}
          />
          
          {/* Drawer Content */}
          <aside className="relative flex w-64 max-w-[80vw] h-full flex-col border-r border-charcoal bg-obsidian p-4 z-10 transition-transform duration-300">
            {/* Header Drawer */}
            <div className="flex items-center justify-between pb-4 mb-4 border-b border-charcoal">
              <div className="flex items-center gap-2">
                <div className="flex h-6 w-6 items-center justify-center rounded bg-green">
                  <Scan size={13} className="text-obsidian" />
                </div>
              ...
              <button
                type="button"
                onClick={() => setIsMobileMenuOpen(false)}
                className="p-1 text-smoke hover:text-snow hover:bg-ash rounded-lg transition-colors"
            ...
            {/* User + logout */}
            <div className="border-t border-charcoal pt-4 mt-auto">
              {user && (
      ...
      <aside className="hidden lg:flex w-56 flex-col border-r border-charcoal bg-obsidian shrink-0">
        {/* Logo */}
      ...
      {/* Contenido principal */}
      <main className="flex-1 overflow-y-auto bg-obsidian w-full">
        <Outlet />
      </main>
    </div>
  );
