...
2. Toda comunicación inter-módulos que implique "reacción" (ej. "cuando un estudio se analiza, crear un reporte") debe realizarse publicando y suscribiendo al `EventDispatcher` de Hexcore (`AnalisisCompletadoEvent`), con su propio `UnitOfWork`.
3. Cualquier componente CPU-bound (compresión de imágenes, inferencia IA, generación de PDFs densos) debe aislarse en infraestructura usando `loop.run_in_executor()` para no detener el *event loop* principal de FastAPI.